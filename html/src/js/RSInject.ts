...
export default class RSInject {
  ...
  private static getSpeakerUrl(readclass: string) {
    const url = new URL(this.config.read_url);
  ...
  private static readFirstSpeakerLink() {
    ...
    if (this.enabled) {
      const speakerLink = document.querySelector(
        "." + this.config.speaker_class
  ...
  private static enableZoneElement(
    zonekey: string,
    zone: Zone,
    ...
    if (
      !zone.greedy &&
      containerElement.classList.contains(this.config.zone_class)
    ) {
    ...
    containerElement.classList.add(
      this.config.zone_class,
      this.config.zone_class + "-" + zonekey
    );
    ...
    const speakerElement = containerElement.querySelector(
      zone.speaker_selector + " ." + this.config.speaker_class
    );
    if (!speakerElement) {
      if (speakerContainer) {
        if (zone.speaker_position === "after") {
          speakerContainer.append(
            this.getSpeakerLink(readclass, zone.speaker_class)
          );
        } else if (zone.speaker_position === "before") {
          speakerContainer.prepend(
            this.getSpeakerLink(readclass, zone.speaker_class)
          );
        }
      }
      ...
      this.debug(
        "enableZoneElement: element already contains a speakerElement"
      );
    }
  }

  static disableAllZoneElements() {
    Array.from(document.getElementsByClassName(this.config.zone_class)).forEach(
      (elem) => {
        ...
        // remove read classes
        Array.from(elem.getElementsByClassName(this.config.read_class)).forEach(
          (subelem) => {
            const toRemove = [];
        ...
        // restore aria-labels
        Array.from(elem.getElementsByClassName("rsi-aria-hidden")).forEach(
          (subelem) => {
        ...
        const links = elem.getElementsByClassName(this.config.speaker_class);
